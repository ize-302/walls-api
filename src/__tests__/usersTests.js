...
export const getUserStatsTests = () => {
  ...
  describe('Given an invalid username', () => {
    it('should respond with 404 status code', async () => {
      const response = await agent.get(`${BASE_PATH}/users/test--non-existing-user/stats`)
      expect(response.statusCode).toBe(404)
    });
  });
  describe('Given a valid username', () => {
...
export const handleUserFollowTests = () => {
  ...
  describe('If user does not exists', () => {
    it('should respond with 404 status code', async () => {
      const response = await agent.post(`${BASE_PATH}/${path}`).query({ 'username': 'non-existing-user' }).set('Cookie', await handleTestUserLogin(user1Credentials))
      expect(response.statusCode).toBe(404)
    });
  })
  ...
  describe('If successfuly followed user', () => {
    it('should respond with 200 status code', async () => {
      const currentUserPreviousStats = await agent.get(`${BASE_PATH}/users/user1/stats`)
      const targetUserPreviousStats = await agent.get(`${BASE_PATH}/users/user2/stats`)
      const response = await agent.post(`${BASE_PATH}/${path}`).query({ 'username': user2Credentials.username }).set('Cookie', await handleTestUserLogin(user1Credentials))
      const currentUserUpdatedStats = await agent.get(`${BASE_PATH}/users/user1/stats`)
      const targetUserUpdatedStats = await agent.get(`${BASE_PATH}/users/user2/stats`)
